(() => {
    var home = require("Storage").readJSON("home.json");
    ...
    Bangle.on("swipe", dir => {
      ...
      } else if ((home.current > 0 && dir > 0) || (home.current < home.apps.length && dir < 0)) {
        home.current -= dir;
        Terminal.println(home.current);
        Terminal.println(home.apps[home.current]);
        require("Storage").writeJSON("home.json", home);
        load(home.apps[home.current]);
      }
    });
    Bangle.on('touch', button => {
        const command = state.music === "play" ? "pause" : "play"
        gbSend({ t: "music", n: command });
    });
    setWatch(() => {
      if (state.music === "play") {
